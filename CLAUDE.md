...
- `npm run db:seed` — run [Back/prisma/seed.js](Back/prisma/seed.js) (seeds 4 roles, the internal users `admin`/`admin123` (ADMIN), `dueno`/`dueno123` (OWNER) and `instructor`/`instructor123` (STAFF), 3 plans, demo clients with subscriptions/payments/attendance, and demo store products so the dashboard/tienda have data). Idempotent — also the apply step for the `OWNER` role on a live DB, since `roles` is a plain seeded table (no `prisma migrate`).
...
Express app composed in [Back/src/app.js](Back/src/app.js) (`createApp()`), listened on by [Back/src/server.js](Back/src/server.js) (which also handles graceful shutdown). All routes mount under `/api` ([Back/src/routes/index.js](Back/src/routes/index.js)): `health`, `auth`, `dashboard`, `plans`, `clientes`, `suscripciones`, `pagos`, `asistencias`, `productos`, `ventas`, `usuarios`. Guard tiers: `usuarios` and **plan writes** are **ADMIN+OWNER** (`requireRole("ADMIN", "OWNER")`); the staff resources use **ADMIN+OWNER+STAFF**. For `usuarios`, OWNER is further scoped **in the controller** to only manage `STAFF`/`CLIENT` accounts (never `ADMIN`/`OWNER`) — see `puedeGestionarRol` in [usuarios.controller.js](Back/src/controllers/usuarios.controller.js).

Layering convention — keep it:

- **routes/** — only wire URLs to controllers and apply middleware. The standard guard for staff-only resources is `const staff = [requireAuth, requireRole("ADMIN", "OWNER", "STAFF")]` spread into each route; every controller method is wrapped in `asyncHandler`.
...
- **lib/** — cross-cutting helpers: `prisma.js` (singleton libSQL-adapter client, reused on hot reload), `crud.js` (the CRUD factory), `pagination.js` (`parsePagination`/`buildPaginationMeta`), `httpError.js` (`HttpError`), `asyncHandler.js`, `logger.js` (a **pino** instance + a `pino-http` request-logging middleware). `logger.js` exports `logger` (use `req.log` inside a request — it carries the per-request id — and fall back to `logger` elsewhere) and `httpLogger` (mounted first in `app.js` so every response is logged with level by status: 5xx→error, 4xx→warn, else info). It redacts the `authorization`/`cookie` headers and trims response headers to just `statusCode`; pretty-prints only when `NODE_ENV=development` (raw JSON in prod/test). Level comes from `env.logLevel` (`LOG_LEVEL` env, default `debug` in dev / `info` in prod). Prefer it over `console.*` for any new server-side logging.
...
- **middleware/** — `auth.js` (`requireAuth` reads `Authorization: Bearer <jwt>` and sets `req.user`; `requireRole(...roles)`), `error.js`, `rateLimit.js` (`apiLimiter` mounted on all of `/api`, stricter `authLimiter` on `/auth/login`+`/auth/register`). The app also uses `helmet()` for security headers and a `100kb` JSON body limit, and sets `trust proxy: 1` so rate limiting sees the real client IP behind Railway/Vercel.

**The CRUD factory.** [Back/src/lib/crud.js](Back/src/lib/crud.js) — `crudController(modelName, { schema, label, include, orderBy, softDelete })` returns `{ list, get, create, update, remove, findOr404 }` for a Prisma model. It enforces **soft delete by default**: `list`/`get` filter `deletedAt: null`, and `remove` sets `deletedAt` instead of deleting the row. `update` validates with `schema.partial()`. **Pagination is opt-in** via the `parsePagination`/`buildPaginationMeta` helpers in [Back/src/lib/pagination.js](Back/src/lib/pagination.js): a `list` request with `?page`/`?limit` returns `{ items, pagination }` (limit capped at 100); without those params it returns `{ items }` (all rows) exactly as before, so the current frontend stays compatible. Reuse the same helpers in any hand-written `list` (as `usuarios.controller.js` does) rather than re-deriving the math. The `clientes`, `pagos`, `asistencias`, and `productos` controllers are thin wrappers around this; reach for it before hand-writing CRUD. [ventas.controller.js](Back/src/controllers/ventas.controller.js) reuses the factory's `list`/`get` but hand-writes `create`/`remove` inside a `prisma.$transaction` so a sale decrements `Producto.stock` (and voiding a sale restores it) atomically — the only place interactive transactions are used.
...
- `<DashboardLayout/>` ([layouts/DashboardLayout/](src/Components/layouts/DashboardLayout/)) wraps the admin panel (`/Dashboard` index + `/Dashboard/Asistencias` + `/Dashboard/Ingresos` + `/Dashboard/Usuarios`) — a collapsible sidebar (links grouped into "Administración" / "Sitio" sections), no top nav. It's gated by `<RequireRole roles={['ADMIN','OWNER','STAFF']}>` ([routing/RequireRole.jsx](src/Components/routing/RequireRole.jsx)), which renders nothing while auth is resolving, redirects to `/Login` when logged out, and to `/` when the role isn't allowed. `/Dashboard/Usuarios` is **further** wrapped in `<RequireRole roles={['ADMIN','OWNER']}>` (STAFF can't reach it). The sidebar is **role-driven**: `buildNavGroups(role)` builds the "Administración" group from `ADMIN_ORDER_BY_ROLE`, so the order and visibility of items change per role (ADMIN/OWNER see Panel→Ingresos→Asistencias→Usuarios; STAFF sees Asistencias→Ingresos→Panel and no Usuarios) — there is no longer an `adminOnly` flag.
